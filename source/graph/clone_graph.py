...
from definition.graph_node  import GraphNode
...
class Cloner:
    ...
    def cloneGraph(self, node: Optional['GraphNode']) -> Optional['GraphNode']:
        ...
        if len(node.neighbors) == 0:
            return GraphNode(node.val)

        # >1 node graph
        orig_clone_map = {}
        def cloneNode(originalNode):
            if originalNode in orig_clone_map:
                return orig_clone_map[originalNode]

            clone = GraphNode(originalNode.val)
            orig_clone_map[originalNode] = clone
