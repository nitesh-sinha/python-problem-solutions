...
class CourseScheduleChecker:
    ...
    def canFinish(self, numCourses: int, prerequisites: list[list[int]]) -> bool:
        ...
        for course, course_prereq in prerequisites:
            prereq_adj_list[course_prereq].append(course)
            indegree[course] += 1

        for course_num in range(len(indegree)):
            if indegree[course_num] == 0:
                no_prereq_courses.append(course_num)

        num_courses_taken = 0
        while no_prereq_courses:
            cur_course = no_prereq_courses.pop()
            num_courses_taken += 1
            for course in prereq_adj_list[cur_course]:
                indegree[course] -= 1
