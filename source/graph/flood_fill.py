...
#     side with the original pixel, either horizontally or vertically) and shares the same color 
...
class Filler:
    ...
    def floodFill(self, image: list[list[int]], sr: int, sc: int, color: int) -> list[list[int]]:
        rows = len(image)
        cols = len(image[0])
        flood_filled = [[False for _ in range(cols)] for _ in range(rows)]
        if image[sr][sc] == color:
            # start pixel is already of the desired color
            return image
        self.fillColour(image, flood_filled, sr, sc, color, image[sr][sc])
        return image

    
    def fillColour(self, image, flood_filled, sr, sc, color, start_color):    
        if sr >= len(image) or sr < 0 or sc >= len(image[0]) or sc < 0 or flood_filled[sr][sc]:
            # either pixel crossed the image boundary or pixel is already flood-filled
            return

        if image[sr][sc] == start_color:
            # Flood fill this pixel
            # and recursively go to neighbours
            image[sr][sc] = color
            flood_filled[sr][sc] = True
            self.fillColour(image, flood_filled, sr+1, sc, color, start_color)
            self.fillColour(image, flood_filled, sr-1, sc, color, start_color)
            self.fillColour(image, flood_filled, sr, sc+1, color, start_color)
            self.fillColour(image, flood_filled, sr, sc-1, color, start_color)
