...
class BTreePrinter:
    @staticmethod
    def print_node(root: TreeNode):
        max_level = BTreePrinter.max_level(root)
        BTreePrinter.print_node_internal([root], 1, max_level)

    @staticmethod
    def print_node_internal(nodes, level, max_level):
        if not nodes or BTreePrinter.is_all_elements_null(nodes):
            return
    ...
    def max_level(node):
        ...
        return max(BTreePrinter.max_level(node.left), BTreePrinter.max_level(node.right)) + 1
